...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    static {
        ...
        HeartTaskJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
    ...
    static {
        ...
        DI_RECTANGLES.add(STENCIL_TASK_HEART);
    ...
    public ObjectNode convertToJson(BpmnModel model) {
        ...
        if (model.getPools().size() > 0) {
            for (Pool pool: model.getPools()) {
                ...
                ObjectNode poolNode =
                        BpmnJsonConverterUtil.createChildShape(pool.getId(), STENCIL_POOL, graphicInfo.getX()
                                + graphicInfo.getWidth(), graphicInfo.getY() + graphicInfo.getHeight(),
                ...
                if (process != null) {
                    for (Lane lane: process.getLanes()) {
                        ...
                        ObjectNode laneNode =
                                BpmnJsonConverterUtil.createChildShape(lane.getId(), STENCIL_LANE,
    ...
    public void processFlowElements(Collection<FlowElement> flowElements, BpmnModel model,
    ...
    public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement,
        ...
        for (JsonNode shapeNode: shapesArrayNode) {
            Class<? extends BaseBpmnJsonConverter> converter =
                    convertersToBpmnMap.get(BpmnJsonConverterUtil.getStencilId(shapeNode));
            if (converter != null) {
                try {
    ...
    private void postProcessElements(Process process, Collection<FlowElement> flowElementList,
        ...
        for (FlowElement flowElement: flowElementList) {
            ...
            if (flowElement instanceof BoundaryEvent) {
                ...
                Activity activity =
                        retrieveAttachedRefObject(boundaryEvent.getAttachedToRefId(),
        ...
        if (objectNode.get(EDITOR_CHILD_SHAPES) != null) {
            for (JsonNode jsonChildNode: objectNode.get(EDITOR_CHILD_SHAPES)) {
                ...
                if (STENCIL_SEQUENCE_FLOW.equals(stencilId) == false) {
                    ...
                    graphicInfo
                            .setWidth((lowerRightNode.get(EDITOR_BOUNDS_X).asDouble() - graphicInfo.getX())
                                    + parentX);
                    graphicInfo.setHeight((lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble() - graphicInfo
                            .getY()) + parentY);
                    ...
                    readShapeDI(jsonChildNode, graphicInfo.getX(), graphicInfo.getY(), shapeMap,
                            sourceRefMap, bpmnModel);
                }
            }
        }
    }
    ...
    private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap,
        ...
        for (String edgeId: edgeMap.keySet()) {
            ...
            if (DI_CIRCLES.contains(sourceRefStencilId)) {
                Circle2D eventCircle =
                        new Circle2D(sourceInfo.getX() + sourceDockersX, sourceInfo.getY() + sourceDockersY,
                                sourceDockersX);
            ...
            if (dockersNode.size() > 2) {
                ...
                double startLastLineX =
                        dockersNode.get(dockersNode.size() - 2).get(EDITOR_BOUNDS_X).getDoubleValue();
                double startLastLineY =
                        dockersNode.get(dockersNode.size() - 2).get(EDITOR_BOUNDS_Y).getDoubleValue();
                
                double endLastLineX =
                        dockersNode.get(dockersNode.size() - 1).get(EDITOR_BOUNDS_X).getDoubleValue();
                double endLastLineY =
                        dockersNode.get(dockersNode.size() - 1).get(EDITOR_BOUNDS_Y).getDoubleValue();
            ...
            if (DI_RECTANGLES.contains(targetRefStencilId)) {
                ...
                Circle2D eventCircle =
                        new Circle2D(targetInfo.getX() + targetDockersX, targetInfo.getY() + targetDockersY,
                                targetDockersX);
    ...
    private Polyline2D createRectangle(GraphicInfo graphicInfo) {
        Polyline2D rectangle =
                new Polyline2D(new Point2D(graphicInfo.getX(), graphicInfo.getY()), new Point2D(
                        graphicInfo.getX() + graphicInfo.getWidth(), graphicInfo.getY()), new Point2D(
                        graphicInfo.getX() + graphicInfo.getWidth(), graphicInfo.getY()
                                + graphicInfo.getHeight()), new Point2D(graphicInfo.getX(),
                        graphicInfo.getY() + graphicInfo.getHeight()), new Point2D(graphicInfo.getX(),
                        graphicInfo.getY()));
        return rectangle;
    }
    ...
    private Polyline2D createGateway(GraphicInfo graphicInfo) {
        ...
        Polyline2D gatewayRectangle =
                new Polyline2D(new Point2D(graphicInfo.getX(), middleY), new Point2D(middleX,
                        graphicInfo.getY()),
                        new Point2D(graphicInfo.getX() + graphicInfo.getWidth(), middleY), new Point2D(
                                middleX, graphicInfo.getY() + graphicInfo.getHeight()), new Point2D(
